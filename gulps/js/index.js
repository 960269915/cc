...
let mask = {
  ...
  addMask(text = "测试") {
    let p = $(
      `<p class="mask-item"><span class="mask-item-text">${text}</span></p>`
    );
    this.tmp.prepend(p);
    this.move();
  },
...
let msg = {
  ...
  text(url, name, text, lr) {
    let text_str = $(`<div id="msg-box" data-id="${user.id}" class="msg-box ${
      lr == "left" ? "flex" : ""
    }">
                   ...
                   <div class="msg-wrap ${lr == "left" ? "left" : "fl right"} ">
                     ...
                     <p class="msg-text ${
                       lr == "left" ? "admin-color" : "user-color"
                     }">${text}</p>
                   </div>
                   </div`);
    return text_str;
  ...
  img(url, name, imgurl, lr) {
    let img_str = $(`<div id="msg-box" data-id="${user.id}" class="msg-box ${
      lr == "left" ? "flex" : ""
    }">
                   <img class=" user-photo ${
                     lr == "left" ? "" : "fr"
                   }" src="${url}" alt="">
                   <div div class = "msg-wrap ${
                     lr == "left" ? "left" : "fl right"
                   } " >
                     <p class="user-name ${lr == "right" ? "tr" : ""}">${name} 
                     <span class="admin-tip">${
                       user.type == "admin" ? "管理员" : ""
  ...
  audio(url, name, audiourl, lr) {
    let audio_str = $(`<div id="msg-box" data-id="${user.id}" class="msg-box ${
      lr == "left" ? "flex" : ""
    }">
                   <img class=" user-photo ${
                     lr == "left" ? "" : "fr"
...
$("#icon-box").on("click", "li", function () {
  ...
  let newval =
    val.slice(0, positions) + emoji + val.slice(positions, val.length);
...
function getmsg(data) {
  ...
  // add
  if (dataObj.type == "userCount") {
    let userNum = Number($("#watch-user").text());
    userNum = dataObj.info == "in" ? userNum + 1 : userNum - 1;
    $("#watch-user").text(userNum);
    return;
  }
  // add
  if (dataObj.type == "remove") {
    if (navIndex == 2) {
      let index = $("#content2 .msg-box").index($(msgEx.removeEle));
      $("#mask-wrap p").eq(index).remove();
    }
    $(msgEx.removeEle).remove();
  ...
  if (dataObj.type == "text") {
    textele = msg.text(user.photo, user.name, dataObj.msg, "left");
  }
  if (dataObj.type == "jpg") {
