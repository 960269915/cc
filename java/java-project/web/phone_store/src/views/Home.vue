        ...
        >
          ...
          >
            ...
            >
              ...
              <template #footer>
                <van-button round type="info" size="mini" @click="buy(index)"
                  >购买</van-button
                >
              </template>
            </van-card>
...
export default {
  ...
  created() {
    ...
    axios.get("http://localhost:8181/phone/index").then(function(resp) {
      console.log(resp);
      _this.phones = resp.data.data.phoneVos;
      _this.categories = resp.data.data.categoryVos;
    });
  },
  methods: {
    onClick(index) {
    ...
    onBuyClicked(item) {
      ...
      localStorage.setItem("specsId", item.selectedSkuComb.s1);
      localStorage.setItem("quantity", item.selectedNum);
